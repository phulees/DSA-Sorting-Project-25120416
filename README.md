...
* **Đối với bài `strlexi`:** 
  * Thay vì sao chép chuỗi tốn kém (việc gán chuỗi dài 100 ký tự mất $O(L)$ thời gian), tôi chuyển sang **sắp xếp con trỏ chuỗi** (`vector<const string*> ptrs`). Khi đổi chỗ (swap) hoặc trộn (merge), chương trình chỉ thao tác trên địa chỉ bộ nhớ với chi phí $O(1)$ thay vì dữ liệu chuỗi thực tế. Mảng `temp` cũng được dùng chung tương tự bài `int`.
